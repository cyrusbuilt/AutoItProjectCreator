...
; #VARIABLES# ======================================================================================================================
Global Const $CB_FILE_LIB_VER           = "1.0.2.0"
;Operation constants.
...
Func _FileExtension($path)
	If (StringLen($path) == 0) Then Return SetError(1, 0, "")
...
Func _FileCompare($file1, $file2)
	If ((StringLen($file1) == 0) Or (Not FileExists($file1)) Or (_FileIsDir($file1))) Then Return SetError(1, 0, False)
	If ((StringLen($file2) == 0) Or (Not FileExists($file2)) Or (_FileIsDir($file2))) Then Return SetError(2, 0, False)
...
Func _DiskInfoToArray($drive)
	...
	$aInfo[0] = "INVAlID"
	If (Not _DiskExist($drive)) Then Return SetError(1, 0, $aInfo)
...
Func _FileRenameDlg($oldName, $newName, $flags = 0)
	...
	$oldName = _TrimTrailingDelimiter($oldName)
	$newName = _TrimTrailingDelimiter($newName)
...
Func _FileCheckDirEmpty($path)
	...
	If (Not _FileIsDir($path)) Then Return SetError(1, 0, $bDirEmpty)
	If (StringRight($path, 1) <> "\") Then $path = $path & "\"
	$hSearchHandle = FileFindFirstFile($Path & "*.*")
	If ((@error) Or ($hSearchHandle == -1)) Then $bDirEmpty = True
...
Func _FileTruncateName($fileName)
	...
	While FileExists($justDir & $name)
		...
		If ($aSegments[0] == 2) Then $name &= "." & $aSegments[2]
		$i += 1
	WEnd
...
Func _FileGetProfilesDir()
	...
	If ((@error) Or (StringLen($result) == 0)) Then SetError(1)
	$key = 0
	$val = 0
	Return $result
EndFunc
